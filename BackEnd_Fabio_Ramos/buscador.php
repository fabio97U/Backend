<?php
  ...
  foreach($data as $i){
      ...
      if($p>=$min && $p<=$max) array_push($r, $i);
  }

  $result = array();
  if(!empty($ciudad) && !empty($tipo)){
      foreach($r as $e)
          if($e['Ciudad']==$ciudad && $e['Tipo']==$tipo) array_push($result, $e);
  } elseif(!empty($ciudad)){
      foreach($r as $e)
          if($e['Ciudad']==$ciudad) array_push($result, $e);
  } elseif(!empty($tipo)){
      foreach($r as $e)
          if($e['Tipo']==$tipo) array_push($result, $e);
